...
<body style="display: flex; flex-direction: column; align-items: center;">
	<h1>이메일 중복체크</h1>
		...
		int result = dao.joinEmailCheck(email);
		%>
		...
		<fieldset>
			...
			<form action="joinEmailCheck.jsp" method="post" name="joinCheck">
				<div style="display: flex; align-items: center;">
					<div style="margin-left: 1em; margin-right: 1em;">이메일</div>
		...
		if (result == 1){
			...
			%>
				<div style="display: flex; align-items: center; flex-direction: column;">
					 ...
					 <input type="button" value="이메일 사용하기" onclick="result();" style="line-height:0;">
				</div>
				
				<script>
				opener.document.join.echk.value = "1";
				</script>
			<%
			
		}else if(result == 0){
			%>
				<div style="color: red;">중복된 이메일입니다.</div>
				<script>
				opener.document.join.echk.value = "0";
				</script>
			<%
		}else{
			out.print("에러 발생!!!(-1)"); 
		...
		<!-- 6. 선택된아이디는 중복확인창에서 회원가입 페이지로 정보전달  -->
		 <script type="text/javascript">
		    function result(){
		    	...
		    	opener.document.join.email.value = document.joinCheck.useremail.value;
